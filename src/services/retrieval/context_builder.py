...
RELATIONSHIPS are never trimmed — RETRIEVAL-003 already caps the ranked
subgraph to top_k (default 20) nodes, so that section staying under 8K
tokens on its own is the practical case, not the exception.
ponytail: if a pathologically large subgraph ever did blow the budget by
itself, this doesn't further trim entities/relationships — upgrade path is
dropping the lowest-ranked ones first, same greedy idea already used for
RELEVANT TEXT CHUNKS below. Chunks (already ranked best-first by
vector_retriever/Chroma) are added one at a time, checking the real
token count of the whole candidate text each time, stopping the moment
one more would cross the limit — exact against what actually gets sent,
not an approximation from summed per-line costs.
"""
