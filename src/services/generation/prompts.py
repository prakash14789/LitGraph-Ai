...
- USES_METHOD: the paper uses/applies a method (its own or an existing one) as part of its approach. Do NOT use this for a method the paper only discusses as related work — a survey-style sentence that names existing approaches to a problem ("existing solutions include X and Y", "prior approaches use X", "methods like X and Y have been proposed") is describing what OTHERS did, not what THIS paper does, even without an explicit "unlike X"/"in contrast to Y" contrast phrase. Only extract USES_METHOD when the text says or clearly implies the paper's own approach actually employs the method — leave it out entirely rather than force it here.
...
- INTRODUCES: the paper is the ORIGINAL SOURCE of this method/concept — it explicitly claims "we introduce/propose X" about ITS OWN novel contribution. Do NOT use this for a method the paper merely uses, extends, cites, or compares against — e.g. a paper using an existing model as a baseline does NOT introduce that baseline, and a paper that reimplements or compares against an existing training objective/technique does NOT introduce that objective/technique, even if the paper discusses it in detail. A paper that presents a replication study, re-implementation, or empirical analysis of an existing method does NOT introduce that method, even if it is the primary subject of the paper — heavy discussion of a method is not the same as originating it. The target name MUST actually appear in the evidence_text you cite for it — never attach an INTRODUCES relationship to a name that isn't in the very sentence you're quoting as evidence. If the text is ambiguous about who originated it, leave INTRODUCES out rather than guess.
...
Negative example of the INTRODUCES trap (do NOT do this): text says "Our best model achieves state-of-the-art results on GLUE, RACE and SQuAD." and, elsewhere in the entity list, an unrelated existing baseline "Method Z" appears. Do NOT emit INTRODUCES→Method Z with that GLUE/RACE/SQuAD sentence as evidence_text — Method Z is never named in that sentence, so it is not grounded. This kind of mismatch (a plausible-looking name pulled from elsewhere in the section, attached to a results sentence that never mentions it) is exactly the mistake to avoid.

Negative example of the USES_METHOD trap (do NOT do this): text says "Existing solutions to this problem include Method X and Method Y." followed later by "We instead propose Method Z, which avoids the overhead of prior approaches." Do NOT emit USES_METHOD→Method X or USES_METHOD→Method Y — that sentence introduces them only as other people's prior work being surveyed before the paper's own (different) proposal, not as something this paper's approach uses.

Second negative example of the USES_METHOD trap, list form (do NOT do this — this exact shape is easy to miss): text says "Self-training approaches such as Method A (Smith et al., 2018), Method B (Jones et al., 2018), and Method C (Lee et al., 2019) have brought significant performance gains, but it can be challenging to determine which aspects contribute most." Do NOT emit USES_METHOD for Method A, Method B, or Method C from that sentence — a citation-heavy list crediting several different prior papers' methods with a general trend ("have brought gains") is a related-work survey clause, not a statement that this paper's own approach employs any of them, even though each name is clearly and unambiguously mentioned in the text.
